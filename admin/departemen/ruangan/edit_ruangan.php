...
try {
    // Mengambil data ruangan yang akan diedit
    $sql_ruangan = "SELECT * FROM tb_ruangan WHERE id_ruangan = :id_ruangan";
    $stmt_ruangan = $conn->prepare($sql_ruangan);
    $stmt_ruangan->bindParam(':id_ruangan', $id_ruangan);
    $stmt_ruangan->execute();
    $ruangan = $stmt_ruangan->fetch(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Edit Ruangan - RSUD Bakti Permana</title>
...
<body>
    ...
    <div class="app-container">
        ...
        <main class="main-content position-relative">
            ...
            <a href="../detail.php?id=<?= $ruangan['id_departemen'] ?>" class="btn btn-sm btn-light border text-muted mb-4 d-inline-flex align-items-center gap-2" style="border-radius: 0.5rem; text-decoration: none;">
                <i data-lucide="arrow-left" size="16"></i> Batal & Kembali
            </a>

            <div class="mb-4">
                <h4 class="fw-bold mb-1" style="color: var(--primary-text);">Edit Ruangan</h4>
                <span style="font-size: 0.85rem; color: var(--muted-text);">Ubah informasi ruangan yang dipilih.</span>
            </div>
                ...
                <form action="edit_proses.php" method="POST">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="id_departemen" class="form-control-custom" style="appearance: auto;" required>
                                ...
                                <?php foreach($departemens as $dept): ?>
                                    <option value="<?= $dept['id_departemen'] ?>" <?= ($ruangan['id_departemen'] == $dept['id_departemen']) ? 'selected' : '' ?>>
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" name="nama_ruangan" class="form-control-custom" placeholder="Misal: Ruang Anggrek 01 atau ICU-B" value="<?= htmlspecialchars($ruangan['nama_ruangan']) ?>" required>
                        </div>

                        <div class="col-md-6">
                            <label class="custom-label">Kapasitas Kasur (Bed) <span class="text-danger">*</span></label>
                            <input type="number" name="kapasitas_maksimal" class="form-control-custom" placeholder="Misal: 4" min="1" value="<?= $ruangan['kapasitas_maksimal'] ?>" required>
                        </div>

                        <div class="col-md-6">
                            <label class="custom-label">Status Ketersediaan <span class="text-danger">*</span></label>
                            <select name="status_ketersediaan" class="form-control-custom" style="appearance: auto;" required>
                                <option value="Tersedia" <?= ($ruangan['status_ketersediaan'] == 'Tersedia') ? 'selected' : '' ?>>Tersedia</option>
                                <option value="Penuh" <?= ($ruangan['status_ketersediaan'] == 'Penuh') ? 'selected' : '' ?>>Penuh</option>
                                <option value="Maintenance" <?= ($ruangan['status_ketersediaan'] == 'Maintenance') ? 'selected' : '' ?>>Maintenance (Perbaikan)</option>
                            </select>
                        </div>
                        ...
                        <div class="col-12 mt-5 d-flex gap-3">
                            ...
                            <a href="../detail.php?id=<?= $ruangan['id_departemen'] ?>" class="btn-cancel">
                                Batal
                            </a>
                        </div>
