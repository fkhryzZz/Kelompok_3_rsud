...
<head>
    ...
    <title>Edit Dokter - RSUD Bakti Permana</title>
...
<body>
    ...
    <div class="app-container">
        ...
        <main class="main-content position-relative">
            ...
            <!-- Judul Halaman -->
            <div class="mb-4">
                <h4 class="fw-bold mb-1" style="color: var(--primary-text);">Edit Dokter</h4>
                <span style="font-size: 0.85rem; color: var(--muted-text);">Ubah informasi dokter beserta akun loginnya di sistem.</span>
            </div>

            <!-- Form Card -->
            <div class="form-card">
                <form action="update_proses.php" method="POST">
                    <input type="hidden" name="id_dokter" value="<?= htmlspecialchars($dokter['id_dokter']) ?>">
                    <input type="hidden" name="id_user" value="<?= htmlspecialchars($dokter['id_user']) ?>">
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="email" name="email" class="form-control-custom" placeholder="Misal: dokter@rsud.com" value="<?= htmlspecialchars($dokter['email'] ?? '') ?>" required>
                        </div>

                        <!-- Password (Opsional untuk update) -->
                        <div class="col-md-6">
                            <label class="custom-label">Password Baru (Kosongkan jika tidak ingin mengubah)</label>
                        ...
                        <div class="col-md-12 mt-3">
                            ...
                            <input type="text" name="nama_dokter" class="form-control-custom" placeholder="Misal: Dr. Ahmad Santoso, Sp.PD" value="<?= htmlspecialchars($dokter['nama_dokter'] ?? '') ?>" required>
                        </div>

                        <!-- No. STR -->
                        <div class="col-md-6">
                            <label class="custom-label">Nomor STR <span class="text-danger">*</span></label>
                            <input type="text" name="no_str" class="form-control-custom" placeholder="Misal: 1234567890123456" value="<?= htmlspecialchars($dokter['no_str'] ?? '') ?>" required>
                        </div>

                        <!-- Spesialisasi -->
                        <div class="col-md-6">
                            <label class="custom-label">Spesialisasi <span class="text-danger">*</span></label>
                            <input type="text" name="spesialisasi" class="form-control-custom" placeholder="Misal: Penyakit Dalam" value="<?= htmlspecialchars($dokter['spesialisasi'] ?? '') ?>" required>
                        </div>

                        <!-- Departemen/Poli -->
                        <div class="col-md-12">
                            <label class="custom-label">Departemen/Poli <span class="text-danger">*</span></label>
                            <select name="id_departemen" class="form-control-custom" required>
                                <option value="">-- Pilih Departemen --</option>
                                <?php foreach ($departemens as $departemen): ?>
                                    <option value="<?= htmlspecialchars($departemen['id_departemen']) ?>" 
                                            <?= ($dokter['id_departemen'] == $departemen['id_departemen']) ? 'selected' : '' ?>>
                        ...
                        <div class="col-12 mt-4">
                            <div class="d-flex gap-3">
                                ...
                                <a href="index.php" class="btn-cancel">
                                    <i data-lucide="x" size="18"></i> Batal
                                </a>
                            </div>
                        </div>
